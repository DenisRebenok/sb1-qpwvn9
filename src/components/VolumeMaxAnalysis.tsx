...
import { calculateVolumeMax, bestPracticesRules } from '../utils/volumeMaxCalculations';
...
export const VolumeMaxAnalysis: React.FC<VolumeMaxAnalysisProps> = ({ workoutData }) => {
  ...
  const volumeData = useMemo(() => calculateVolumeMax(workoutData, days), [workoutData, days]);

  const handleDaysChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setDays(parseInt(event.target.value));
  };

  const getRowColor = (current: number, muscleGroup: string) => {
    const rule = bestPracticesRules[muscleGroup];
    if (!rule) return '';
    if (current < rule.min) return 'bg-red-100';
    if (current >= rule.max) return 'bg-green-100';
          ...
          {Object.entries(volumeData).map(([muscleGroup, data]) => {
            ...
            return (
              <tr 
                ...
                className={`cursor-pointer hover:bg-gray-100 ${getRowColor(data.currentVolume, muscleGroup)}`}
              >
